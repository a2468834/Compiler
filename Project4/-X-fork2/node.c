...
void addChild(struct nodeType *node, struct nodeType *new_child)
{
  new_child->parent = node;

  // 'node->child' means old 'child' of 'node'
  // if this node does not have any child
  if(node->child == NULL)node->child = new_child;

  else
  {
    new_child->next = node->child->next;
    new_child->prev = node->child;

    node->child->next->prev = new_child;
    node->child->next = new_child;
  }
}
/*
